...
const Community = ({ userId }) => {
  ...
  const handleCreateReply = (commentId) => {
    fetch('https://brain-safari-backend.onrender.com/replies', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ user_id: userId, comment_id: commentId, content: newReply }),
    })
      .then((response) => response.json())
      .then((data) => {
        setComments(
  ...
  const toggleUserComments = () => {
    if (!showUserComments) {
      fetch(`https://brain-safari-backend.onrender.com/comments/user/${userId}/replies`)
        .then((response) => response.json())
    ...
    <div className="community">
      ...
      {showUserComments ? (
        <div className="user-comments">
          <h3>Your Comments</h3>
          {Array.isArray(userComments.comments) && userComments.comments.length > 0 ? (
            userComments.comments.map((comment) => (
              <div key={comment.id} className="comment-card">
                ...
                >
                  {visibleReplies[comment.id] ? 'Hide Replies' : `View Replies (${comment.replies_count})`}
                </button>
